...
# Cadena: addc#   (si no pones '#', se agrega)
...
def parse_grammar():
    print("Introduce la gramática (una línea por no terminal). Ejemplo: S->aAd|aB")
...
def read_input_syms():
    s = input("\nCadena a analizar (se admite con o sin espacios): ").strip().replace(" ", "")
    if not s.endswith("#"):
        s += "#"
    return list(s)
...
def print_state(mode, i, alpha, beta, note):
    print(f"( {mode}, {i}, {alpha_to_str(alpha)}, {beta_to_str(beta)} )".ljust(50), note)
...
def analyze(grammar, start, labels, input_syms):
    ...
    i = 1                 # posición 1-based en la entrada
    actions = []          # historial: {'kind': 'expand'|'match', 'detail':..., 'nonterm':..., 'alt':..., 'rhs':...}
    decisions = []        # pila de decisiones: {'X':..., 'alt':..., 'i0':..., 'alpha_len':..., 'rhs':...}
    ...
    while True:
        ...
        if mode == "n":
            ...
            # Regla 2: concordancia de terminal
            if not X.isupper():
                if X == a:
                    beta = beta[1:]                   # consume terminal de la pila
                    alpha.append(X)                   # agrega a α
                    actions.append({"kind":"match","detail":X})
                    i += 1
                    print_state("n", i, alpha, beta, f"(2) concordancia '{X}'")
                    continue
                else:
                    mode = "r"
                    if X == "#":
        ...
        # ===== MODO 'r' (retroceso) =====
            ...
            # Regla 5: deshacer matches hasta volver al punto de decisión cuando haga falta
            if actions and actions[-1]["kind"] == "match":
                ...
                if alpha and alpha[-1] == last["detail"]:
                    ...
                    # por si no está al final, buscar desde atrás
            ...
            while i > dp["i0"]:
                ...
                i -= 1
                beta = [m["detail"]] + beta
                # quitar terminal correspondiente en alpha (última ocurrencia)
                for p in range(len(alpha)-1, -1, -1):
                    if alpha[p] == m["detail"]:
                        alpha.pop(p)
                        break
            ...
            while q >= 0:
                a = actions[q]
                if a["kind"] == "expand" and a["nonterm"] == X and a["alt"] == tried_alt:
                    actions.pop(q)
                    break
                q -= 1
